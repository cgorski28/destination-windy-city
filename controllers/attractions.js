...
module.exports.createAttraction = async (req, res, next) => {
    ...
    attraction.image = req.files.map(f => ({ url: f.path, filename: f.filename }))
...
module.exports.updateAttraction = async (req, res) => {
    const { id } = req.params;
    const attraction = await Attraction.findByIdAndUpdate(id, { ...req.body.attraction }, { runValidators: true, new: true });
    ...
    if (req.body.deleteImages) {
        ...
        await attraction.updateOne({ $pull: { image: { filename: { $in: req.body.deleteImages } } } })
